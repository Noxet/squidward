                 ...
                 size_t send_buffer_length)
{
  ...
  if (c_session) {
    ...
    if (result != (int)send_buffer_length) {
      coap_log(LOG_WARNING, "coap_network_send failed (%ld != %ld)\n",
                  ...
                  const unsigned char *name, size_t name_len )
{
  ...
  if (setup_data->validate_id_call_back) {
    ...
    if (psk_key == NULL)
      return -1;
    mbedtls_ssl_set_hs_psk(ssl, psk_key->key, psk_key->key_len);
             ...
             const unsigned char *uname, size_t name_len)
{
  ...
  if (i == m_context->psk_sni_count) {
    ...
    uint8_t *name;
    const coap_dtls_spsk_key_t *new_entry;
    ...
    m_context->psk_sni_entry_list[i].psk_key = *new_entry;
    sni_setup_data = c_session->context->spsk_setup_data;
    sni_setup_data.psk_key = *new_entry;
  ...
  if (ret != -1) {
    coap_session_refresh_psk_key(c_session,
                                 &m_context->psk_sni_entry_list[i].psk_key);
    return mbedtls_ssl_set_hs_psk(ssl,
                           m_context->psk_sni_entry_list[i].psk_key.key,
                           m_context->psk_sni_entry_list[i].psk_key.key_len);
  }
  return ret;
}
#endif /* PSK2_PR */
                                                     ...
                                                     coap_dtls_role_t role)
{
  ...
  mbedtls_esp_enable_debug_log(&m_env->conf, 4);
#endif
...
void coap_dtls_session_update_mtu(coap_session_t *c_session UNUSED)
{
}
...
coap_tick_t coap_dtls_get_timeout(coap_session_t *c_session)
{
  coap_mbedtls_env_t *m_env = (coap_mbedtls_env_t *)c_session->tls;
