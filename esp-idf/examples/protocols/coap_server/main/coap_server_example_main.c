...
/* The examples use simple Pre-Shared-Key configuration that you can set via
   ...
   the config you want - ie #define EXAMPLE_PSK_KEY "some-agreed-preshared-key"

   Note: PSK will only be used if the incoming request was set up with a URI
   prefixes with coaps:// instead of coap:// and the CoAP client is using the
   same PSK.
*/
...
static void coap_example_thread(void *p)
{
    ...
    coap_dtls_set_log_level(COAP_LOGGING_LEVEL);
...
void app_main(void)
{
    ...
    xTaskCreate(coap_example_thread, "coap", 1024 * 4, NULL, 5, NULL);
}
